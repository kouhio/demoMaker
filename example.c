...
void load_image(void) {
	...
	image = IMG_Load("icon_mrgreen.bmp");
	// Put transparent color on surface.
	
//	if(image)
		SDL_SetColorKey(image, SDL_SRCCOLORKEY | SDL_RLEACCEL, htoi(0XF8F8F8));	
        ...
        if ( videoFrame )
        {
            ...
            else if ( videoFrame->pts <= getSync() )
            {
                ...
                else if ( videoFrame->surface )
                {
					...
					dst.w = image->w;
					dst.h = image->h;
					SDL_BlitSurface( image, 0, screen, &dst );
